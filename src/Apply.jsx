    ...
    <div style={styles.page}>
      <div style={styles.card}>
              ...
              <div style={styles.fieldGroup}>
                ...
                <label style={styles.fileLabel}>
                  <input
                    ...
                    style={{ display: 'none' }}
                    onChange={e => set('id_file', e.target.files[0])}
                  />
                  <span style={styles.fileBtn}>
                    {form.id_file ? `✓ ${form.id_file.name}` : '📎 Choose file'}
                  </span>
                </label>
              </div>
              ...
              <div style={styles.fieldGroup}>
                ...
                <label style={styles.fileLabel}>
                  <input
                    type="file" accept="image/*,.pdf"
                    style={{ display: 'none' }}
                    onChange={e => set('income_file', e.target.files[0])}
                  />
                  <span style={styles.fileBtn}>
                    {form.income_file ? `✓ ${form.income_file.name}` : '📎 Choose file'}
                  </span>
                </label>
              </div>
        ...
        <div style={styles.navRow}>
          {step > 0 && (
            <button onClick={() => setStep(s => s - 1)} style={styles.btnBack}>
              ← Back
            </button>
          )}
          <div style={{ flex: 1 }} />
          {step < STEPS.length - 1 ? (
            <button
              ...
              style={{
                ...styles.btnNext,
                opacity: (step === 0 && (!form.first_name || !form.email)) ? 0.4 : 1,
              }}
            >
              Next →
            </button>
...
const styles = {
  ...
  logo: {
    fontFamily: 'Inter,sans-serif', fontSize: 16, fontWeight: 700, letterSpacing: '0.04em',
  },
  headerSub: {
    fontFamily: 'Inter,sans-serif', fontSize: 12, opacity: 0.5, marginTop: 2,
  },
  progress: {
    display: 'flex', padding: '16px 20px', gap: 0,
    borderBottom: '1px solid #f0f0f0', overflowX: 'auto',
  },
  progressStep: {
    display: 'flex', flexDirection: 'column', alignItems: 'center',
    flex: 1, minWidth: 56, gap: 4,
  },
  progressDot: {
  ...
  progressLabel: {
    fontFamily: 'Inter,sans-serif', fontSize: 9, textAlign: 'center', lineHeight: 1.3,
    transition: 'color 0.2s',
  },
  body: { padding: '24px 24px 8px' },
  stepTitle: {
    fontFamily: 'Inter,sans-serif', fontSize: 15, fontWeight: 700,
    marginBottom: 18, color: '#111',
  },
  ...
  label: {
    ...
    color: '#666', letterSpacing: '0.05em', textTransform: 'uppercase',
    marginBottom: 5,
  },
  input: {
    width: '100%', padding: '8px 10px',
    border: '1px solid #e0e0e0', borderRadius: 6,
    fontFamily: 'Georgia, serif', fontSize: 14, color: '#111',
