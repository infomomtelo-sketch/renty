...
If no issues found, return findings as empty array and condition as "Good".`
    ...
    setReport({
      ...
      overall, photoCount: results.length, findings: allFindings,
  ...
  function resetInspection() {
    setPhotos([]); setReport(null); setAddress('')
    setProgress({ done: 0, total: 0 }); setStep('capture')
  }
  ...
  return (
    <div style={{ fontFamily: FONT, background: '#f8f9fb', minHeight: '100vh', paddingBottom: 100 }}>
      ...
      <div style={{ background: '#111', padding: '16px 20px', display: 'flex', alignItems: 'center', gap: 12 }}>
        <button onClick={() => step === 'report' ? resetInspection() : navigate('/dashboard')}
      ...
      {step === 'capture' && (
        <div style={{ padding: '20px 16px', display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ background: '#fff', borderRadius: 14, padding: 16, border: '1px solid #eee' }}>
            <div style={{ fontSize: 11, fontWeight: 600, color: '#aaa', letterSpacing: 0.8, textTransform: 'uppercase', marginBottom: 8 }}>Property Address</div>
            <input value={address} onChange={e => setAddress(e.target.value)} placeholder="123 Main St, Fresno CA"
          ...
          <div style={{ background: '#fff', borderRadius: 14, padding: 16, border: '1px solid #eee' }}>
            <div style={{ fontSize: 11, fontWeight: 600, color: '#aaa', letterSpacing: 0.8, textTransform: 'uppercase', marginBottom: 10 }}>Room / Area</div>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              {ROOMS.map(r => (
                <button key={r} onClick={() => setRoom(r)}
                  style={{ padding: '7px 14px', borderRadius: 99, fontSize: 13, fontFamily: FONT, fontWeight: 500, cursor: 'pointer', border: '1.5px solid', borderColor: room === r ? '#111' : '#e5e5e5', background: room === r ? '#111' : '#fff', color: room === r ? '#fff' : '#555' }}>
                  {r}
                </button>
              ))}
            </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            <button onClick={() => { const inp = document.createElement('input'); inp.type = 'file'; inp.accept = 'image/*'; inp.capture = 'environment'; inp.onchange = e => handleFiles(e.target.files); inp.click() }}
              disabled={photos.length >= 10}
              style={{ padding: '20px 12px', borderRadius: 14, border: '2px dashed #d1d5db', background: '#fff', cursor: photos.length >= 10 ? 'not-allowed' : 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, opacity: photos.length >= 10 ? 0.4 : 1, fontFamily: FONT }}>
              <span style={{ fontSize: 28 }}>📷</span>
              <span style={{ fontSize: 13, fontWeight: 600, color: '#111' }}>Camera</span>
            </button>
            <button onClick={() => { const inp = document.createElement('input'); inp.type = 'file'; inp.accept = 'image/*'; inp.multiple = true; inp.onchange = e => handleFiles(e.target.files); inp.click() }}
              disabled={photos.length >= 10}
              style={{ padding: '20px 12px', borderRadius: 14, border: '2px dashed #d1d5db', background: '#fff', cursor: photos.length >= 10 ? 'not-allowed' : 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, opacity: photos.length >= 10 ? 0.4 : 1, fontFamily: FONT }}>
          ...
          {photos.length > 0 && (
            <div>
              <div style={{ fontSize: 11, fontWeight: 600, color: '#aaa', letterSpacing: 0.8, textTransform: 'uppercase', marginBottom: 10 }}>Photos ({photos.length})</div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8 }}>
                {photos.map(p => (
                  <div key={p.id} style={{ position: 'relative', borderRadius: 10, overflow: 'hidden', aspectRatio: '1', background: '#eee' }}>
                    ...
                    {p.status === 'done' && <div style={{ position: 'absolute', top: 4, left: 4, background: '#16a34a', borderRadius: '50%', width: 20, height: 20, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, color: '#fff' }}>✓</div>}
                    {p.status === 'error' && <div style={{ position: 'absolute', top: 4, left: 4, background: '#dc2626', borderRadius: '50%', width: 20, height: 20, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, color: '#fff' }}>!</div>}
                    <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, background: 'rgba(0,0,0,0.6)', padding: '3px 6px', fontSize: 10, color: '#fff', fontWeight: 500 }}>{p.room}</div>
                    {!analyzing && (
          ...
          {photos.length > 0 && !analyzing && (
            <button onClick={analyzePhotos}
              style={{ width: '100%', padding: '15px 0', background: '#111', color: '#fff', border: 'none', borderRadius: 12, fontSize: 15, fontWeight: 700, cursor: 'pointer', fontFamily: FONT, letterSpacing: -0.2 }}>
            ...
            return (
              <div key={photo.id} style={{ background: '#fff', borderRadius: 14, border: '1px solid #eee', overflow: 'hidden' }}>
                <div style={{ display: 'flex', gap: 12, padding: 14, alignItems: 'center' }}>
                  ...
                  <span style={{ padding: '4px 10px', borderRadius: 99, fontSize: 11, fontWeight: 700, background: SEVERITY_BG[a.condition] || '#f3f4f6', color: SEVERITY_COLOR[a.condition] || '#666', flexShrink: 0 }}>{a.condition}</span>
                </div>
                {a.findings?.length > 0 && (
                  <div style={{ borderTop: '1px solid #f3f4f6' }}>
                    {a.findings.map((f, j) => (
                      <div key={j} style={{ padding: '10px 14px', display: 'flex', gap: 10, alignItems: 'flex-start', borderBottom: j < a.findings.length - 1 ? '1px solid #f9fafb' : 'none' }}>
                        <span style={{ fontSize: 10, fontWeight: 700, padding: '3px 7px', borderRadius: 4, flexShrink: 0, marginTop: 1, background: f.severity === 'Major' ? '#fef2f2' : f.severity === 'Moderate' ? '#fffbeb' : '#f0fdf4', color: f.severity === 'Major' ? '#dc2626' : f.severity === 'Moderate' ? '#d97706' : '#16a34a' }}>{f.severity}</span>
                        <div>
                ...
                {a.findings?.length === 0 && <div style={{ padding: '10px 14px', borderTop: '1px solid #f3f4f6', fontSize: 13, color: '#16a34a' }}>✓ No issues found</div>}
              </div>
            )
          })}
